...
export const TodoScreen = ()=>{
    ...
    const register = async()=>{
        const res= await axios.get('http://localhost:8081/user/'+register);

    }
    const login = async()=>{
        const res= await axios.get('http://localhost:8081/user/'+login);
...
<LoginForm open={isVisible} close={()=>setIsVisible(false)}/>
{/* <SignUpForm/> */}
</>
)
}